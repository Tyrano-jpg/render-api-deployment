...
const GenerateLogsReport = async (data) => {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Logs Reports");
  const columns = [
  ...
  //   { header: "IN", key: "entry_in_time", width: 20 },
  //   // { header: "OUT", key: "entry_out_time", width: 20 },
  ...
  // data.forEach((item) => {
  //   const rowData = {
  //     // entry_out_time: item?.entry_out_time
  //     //   ? moment(item?.entry_out_time).format("h:mm:ss a")
  //     //   : "-",
  //     entry_in_time: item?.entry_in_time
  //       ? moment(item?.entry_in_time).format("h:mm:ss a")
  //       : "-",
  //     room_no: item?.room?.room_no,
  //     session_name: item?.session?.session_name
  //       ? item?.session?.session_name
  //       : "-",
  //     guest: `${item?.guest?.first_name}${item?.guest?.last_name}`,
  //     // duration:
  //     //   item?.entry_in_time && item?.entry_out_time
  //     //     ? moment
  //     //         .utc(
  //     //           moment(item?.entry_out_time).diff(moment(item?.entry_in_time))
  //     //         )
  //     //         .format("H:mm:ss")
  //     //     : "-",
  //   };
  ...
  const filePath = "public/reports/LogsExcel/logs.xlsx";

  await workbook.xlsx.writeFile(filePath);

  const timestamp = new Date().getTime();
  const downloadFileName = `Logs-${timestamp}.xlsx`;

  const destinationPath = `public/reports/LogsExcel/${downloadFileName}`;
