...
export const GetAllCandidateList = async (req, res) => {
  ...
  if (stack && stack !== "null" && stack !== "ALL") {
    filter["stack"] = stack;
  }

  try {
    // const candidates = await applicationModel.find({stack:stack});

    // Fetching candidates with aggregation pipeline
    const candidates = await applicationModel.aggregate([
      { $match: { ...searchQuery, ...filter } },
      { $sort: sort },
      { $skip: skip },
      { $limit: limit },
    ...
    res.status(500).json({
      success: false,
      message: "Server error while grouping candidate list",
    });
  }
};
